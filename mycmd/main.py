...
class CMDStyleTransfer:
    ...
    def run(self, vgg_weights, alpha=0.9, style_loss_weights=(1, 1, 1, 1, 1), epsilon=0.0001, max_iter=500):
        ...
        c_loss = ContentLoss().cuda()
        s_losses = [StyleLoss(conv_s, k=len(style_loss_weights), weights=style_loss_weights).cuda() for conv_s in
                    convs_s]
        outputs = self.model(self.input_img)
        # print(outputs[3].shape)
        # print(torch.nn.ReLU()(outputs[3]).shape)
        ...
        print('loss_c=',loss_c,' loss_s=',loss_s)
