...
from linearmmd import ContentLoss, StyleLoss
# from polymmd import ContentLoss, StyleLoss
from losses import CMDContentLoss, CMDStyleLoss
...
class CMDStyleTransfer:
    ...
    def run(self, vgg_weights, alpha=0.9, style_loss_weights=(1, 1), epsilon=0.0001, max_iter=500):
        ...
        s_losses = [StyleLoss().cuda() for conv_s in convs_s]
        # conv_s, k = len(style_loss_weights), weights = style_loss_weights
        ...
        while iteration < max_iter:
            ...
            for i, (conv_o, conv_s) in enumerate(zip(outputs, convs_s)):
                loss_s += vgg_weights[i] * s_losses[i](torch.sigmoid(conv_o),conv_s)
            ...
            if iteration == 0:
                print("Current iteration is {} and the loss is {}".format(iteration, loss_s.item()))
            iteration += 1
        ...
        CMDc_loss = CMDContentLoss().cuda()
        CMDs_losses = [CMDStyleLoss(conv_s, k=len(style_loss_weights), weights=style_loss_weights).cuda() for conv_s in
                       convs_s]

        outputs = self.model(self.input_img)
        # print(outputs[3].shape)
        # print(torch.nn.ReLU()(outputs[3]).shape)
        CMDloss_c = CMDc_loss(torch.nn.ReLU()(outputs[3]), conv_c)
        CMDloss_s = torch.tensor(0.0).cuda()
        for i, (conv_o, conv_s) in enumerate(zip(outputs, convs_s)):
            CMDloss_s += vgg_weights[i] * CMDs_losses[i](torch.sigmoid(conv_o))
            # print(torch.sigmoid(conv_o).shape)

        print('loss_c=', CMDloss_c, ' loss_s=', CMDloss_s)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--im_size', type=int, default=[423,360], nargs='+', help='Image size. Either single int or tuple of int')
    ...
    if False:
        ...
        saveName = os.path.splitext(os.path.basename(args.c_img))[0] + '-' + os.path.splitext(os.path.basename(args.s_img))[0] + '_mmd'+".png"
