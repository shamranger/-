...
class StyleLoss(nn.Module):
    def __init__(self, target, axis=(0, 2, 3), k=2, weights=(1, 1)):
        ...
        self.axis = axis
        self.rmse = RMSELoss()
        self.c1_y, self.m_y = self._init_targets()
    ...
    def _init_targets(self):
        ...
        m_y = (self.target - c1_y).pow(2).mean(dim=self.axis)
        return c1_y, m_y
    ...
    def __call__(self, x):
        ...
        m_x = (x - c1_x).pow(2).mean(dim=self.axis)
        #     loss = loss + self.weights[i - 1] * self.rmse(m_x, self.m_ys[i - 2])
        loss = loss + self.weights[1] * self.rmse(m_x, self.m_y)
        return loss
